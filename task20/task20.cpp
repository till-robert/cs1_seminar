...
public:
    ...
    signed char flip_if_criterion(int i, int j){ //flip a spin according to criterion
        ...
        else{//kawasaki => M=const, dM=0
            int random_i, random_j;
            do{ //find suitable spin pair
                random_i = random_spin_choice_distrib(gen);
                random_j = random_spin_choice_distrib(gen);
            }while(spinAt(random_i,random_j) == spinAt(i,j));
            //dM = 0;
            signed char dE =   2 * spinAt(i,j) * (spinAt(i-1,j) + spinAt(i+1,j)+ spinAt(i,j-1) + spinAt(i,j+1)); //if i is out of bounds, spinAt(i) returns 0
                             + 2 * spinAt(random_i,random_j) * (spinAt(random_i-1,random_j) + spinAt(random_i+1,random_j)+ spinAt(random_i,random_j-1) + spinAt(random_i,random_j+1)); //if i is out of bounds, spinAt(i) returns 0

            unsigned char dE_index = dE/2 + 8;
            double rand = real_distrib(gen);
            if(rand < flip_probabilities[dE_index]){ //only flip with probability exp(-beta*dE)
                spins[i][j].flip();
                spins[random_i][random_j].flip();
                acceptance_per_sweep ++;
                E_J += dE;
            }
            return spinAt(i,j);
